...
while len(six_letter_guess) != len(secret_word):
    six_letter_guess: str = input(f"That was not {len(secret_word)} letters! Try again: ")
...
while index < len(secret_word): 
    if six_letter_guess[index] == secret_word[index]:
        ...
        while letter_found_anywhere is not True and alternate_indices < len(secret_word):
            if alternate_indices == six_letter_guess[index]: 
                letter_found_anywhere: bool = True
            else: 
                alternate_indices: int = alternate_indices + 1
        if letter_found_anywhere is True: 
